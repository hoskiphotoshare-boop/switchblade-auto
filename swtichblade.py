# =========================================================================
# SWITCHBLADE v47.78 - HEADLESS GITLAB EDITION (PIT + PARQUET)
# =========================================================================
...
class PITUniverseManager:
    ...
    def _clean_ticker(self, symbol):
        ...
        if not s or s in ['NAN', 'NONE', 'CWEN-A']: return None
        if any(char.isdigit() for char in s): return None
    ...
    def _parse_wiki_changes(self, url, curr_table_idx=0, change_table_idx=1, symbol_cols=['Symbol', 'Ticker']):
        try:
            ...
            def get_flat_cols(df):
                if isinstance(df.columns, pd.MultiIndex):
                    return ['_'.join(str(x) for x in col).strip().lower() for col in df.columns.values]
                return [str(col).strip().lower() for col in df.columns.values]

            # 1. Hunt for Current Constituents Table dynamically
            curr_df = tables[curr_table_idx]
            for df in tables:
                flat_cols = get_flat_cols(df)
                if any(s.lower() in flat_cols for s in symbol_cols):
                    curr_df = df
                    if isinstance(curr_df.columns, pd.MultiIndex):
                        curr_df.columns = ['_'.join(str(x) for x in col).strip() for col in curr_df.columns.values]
                    else:
                        curr_df.columns = [str(col).strip() for col in curr_df.columns.values]
                    break

            curr_tickers = set()
            for col in curr_df.columns:
                if str(col).lower() in [s.lower() for s in symbol_cols]:
                    curr_tickers = set(filter(None, [self._clean_ticker(t) for t in curr_df[col].tolist()]))
                    break

            # 2. Hunt for Changes Table dynamically
            change_df = None
            for df in tables:
                flat_cols = get_flat_cols(df)
                if any('date' in c for c in flat_cols) and (any('added' in c for c in flat_cols) or any('removed' in c for c in flat_cols)):
                    change_df = df
                    if isinstance(change_df.columns, pd.MultiIndex):
                        change_df.columns = ['_'.join(str(x) for x in col).strip() for col in change_df.columns.values]
                    else:
                        change_df.columns = [str(col).strip() for col in change_df.columns.values]
                    break
            
            # Fallback URL for S&P 500 Historical spin-off page
            if (change_df is None or len(change_df) < 5) and "500" in url:
                try:
                    r_hist = requests.get("https://en.wikipedia.org/wiki/Historical_components_of_the_S%26P_500", headers=self.headers, timeout=10)
                    hist_tables = pd.read_html(io.StringIO(r_hist.text))
                    for df in hist_tables:
                        flat_cols = get_flat_cols(df)
                        if any('date' in c for c in flat_cols) and (any('added' in c for c in flat_cols) or any('removed' in c for c in flat_cols)):
                            change_df = df
                            if isinstance(change_df.columns, pd.MultiIndex):
            ...
            if change_df is not None:
                cols_lower = [str(c).lower() for c in change_df.columns]
                date_col = next((c for c, c_low in zip(change_df.columns, cols_lower) if 'date' in c_low), None)
                add_col = next((c for c, c_low in zip(change_df.columns, cols_lower) if 'added' in c_low and ('ticker' in c_low or 'symbol' in c_low)), None)
                rem_col = next((c for c, c_low in zip(change_df.columns, cols_lower) if 'removed' in c_low and ('ticker' in c_low or 'symbol' in c_low)), None)
                
                if not add_col: add_col = next((c for c, c_low in zip(change_df.columns, cols_lower) if 'added' in c_low), None)
                if not rem_col: rem_col = next((c for c, c_low in zip(change_df.columns, cols_lower) if 'removed' in c_low), None)

                if date_col:
                    for _, row in change_df.iterrows():
                        try:
                            dt = pd.to_datetime(row[date_col]).date()
                            added = self._clean_ticker(row[add_col]) if add_col and pd.notna(row[add_col]) else None
                            removed = self._clean_ticker(row[rem_col]) if rem_col and pd.notna(row[rem_col]) else None
                            if added or removed:
            ...
            if timeline:
                timeline.append((datetime.date(1900, 1, 1), timeline[-1][1]))

            return timeline, master_tickers
        except Exception as e:
            print(f"      [PIT Warning] Failed parsing {url}: {e}")
            return [(datetime.date.today(), set()), (datetime.date(1900, 1, 1), set())], set()
    ...
    def is_constituent(self, symbol, universe, query_date):
        ...
        for k in keys:
            if k not in self.snapshots or not self.snapshots[k]: continue
            
            oldest_snap_date = self.snapshots[k][-1][0]
            effective_date = query_date if query_date >= oldest_snap_date else oldest_snap_date

            for snap_date, constituents in self.snapshots[k]:
                if effective_date >= snap_date:
